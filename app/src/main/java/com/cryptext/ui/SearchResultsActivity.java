...
public class SearchResultsActivity extends AppCompatActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(final AdapterView<?> parent, final View view,
                ...
                intent.putExtra("location", list.get(position).get(IDBForNotes.POSITION));
                App.setDatabase(db);
    ...
    protected void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
        if (requestCode == DBForNotesAsync.REQUEST_EDIT_NOTE) {
            if (resultCode == RESULT_DELETE) {
                ...
                final Intent intent = getIntent();
                intent.putExtra(IDBForNotes.TITLE, data.getStringExtra(IDBForNotes.TITLE));
                intent.putExtra(IDBForNotes.TAGS, data.getStringExtra(IDBForNotes.TAGS));
                App.setNote(data.getStringExtra(IDBForNotes.NOTE));
    ...
    private void searchList(final String query) {
        ...
        while (iter.hasNext()) {
            ...
            if (!(StringMethods.containsIgnoreCase(item.get(IDBForNotes.TITLE), query) ||
                    StringMethods.containsIgnoreCase(db.getNotes().get(Long.parseLong(item.get(IDBForNotes.POSITION))).getNote(), query))) {
                iter.remove();
            }
        }
    }
