...
public class ChoosePasswordDialogFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(final Bundle savedInstanceState) {
        ...
        db = (IDBForNotes) getArguments().getSerializable("db");
    ...
    private String checkPasswords() {
        ...
        if(fieldEditText.getText().toString().length() < 6 || confFieldEditText.getText().toString().length() < 6){
            Toast.makeText(getActivity(), "Şifre 5 haneden küçük olamaz !", Toast.LENGTH_LONG).show();
            return null;

        }
        if(fieldEditText.getText().toString().startsWith("123") || confFieldEditText.getText().toString().startsWith("123")){
            Toast.makeText(getActivity(), "Kötü şifre seçimi (123 ile başlamasın) ", Toast.LENGTH_LONG).show();
            return null;

        }
        if(fieldEditText.getText().toString().endsWith("123") || confFieldEditText.getText().toString().endsWith("123")){
            Toast.makeText(getActivity(), "Kötü şifre seçimi (123 ile bitmesin) ", Toast.LENGTH_LONG).show();
            return null;
