...
public class ReadNoteActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(final MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.action_starred:
                final Intent intent = getIntent();
                if (isStarred) {
                    intent.putExtra(IDBForNotes.STAR, false);
                    starred.setIcon(R.drawable.ic_star_outline_white_24dp);
                } else {
                    intent.putExtra(IDBForNotes.STAR, true);
                    starred.setIcon(R.drawable.ic_star_white_24dp);
                }
                isStarred = !isStarred;
                intent.putExtra(IDBForNotes.TITLE, db.getNotes().get(location).getTitle());
                intent.putExtra(IDBForNotes.TAGS, db.getNotes().get(location).getTags());
    ...
    protected void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
        ...
        if (requestCode == DBForNotesAsync.REQUEST_EDIT_NOTE && resultCode == RESULT_OK) {
            final Intent intent = getIntent();
